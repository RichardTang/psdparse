...
class PsdLayerParser(PsdLayerImageParser):
  ...
  def parse_layerlen(self, layerlen):
    ...
    if self.num_layers * (18 + 6 * self.header['channels']) > layerlen:
      raise ValueError(
          "Unlikely number of %s layers for %s channels with %s layerlen. Giving up."
          % (self.num_layers, self.header['channels'], layerlen))
    ...
    for i in range(self.num_layers):
      ...
      while self.fd.tell() < extrastart + extralen:
        (
            ...
            key,
            size,
        ) = self._readf(">4s4sL")  # (n,) is a 1-tuple.
